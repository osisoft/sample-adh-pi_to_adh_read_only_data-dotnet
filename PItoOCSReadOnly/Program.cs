...
namespace PItoOCSReadOnly
{
    public static class Program
    {
        ...
        public static async Task<bool> MainAsync(bool test = false)
        {
            ...
            Console.WriteLine(@"-------------------------------------------------------");
            Console.WriteLine(@" _____ _____ _         ____   _____  _____ _____       ");
            Console.WriteLine(@"|  __ \_   _| |       / __ \ / ____|/ ____|  __ \      ");
            Console.WriteLine(@"| |__) || | | |_ ___ | |  | | |    | (___ | |__) |   _ ");
            Console.WriteLine(@"|  ___/ | | | __/ _ \| |  | | |     \___ \|  ___/ | | |");
            Console.WriteLine(@"| |    _| |_| || (_) | |__| | |____ ____) | |   | |_| |");
            Console.WriteLine(@"|_|   |_____|\__\___/ \____/ \_____|_____/|_|    \__, |");
            ...
            try
            {
                ...
                Console.WriteLine("Getting latest event of the stream, note how we can see the PI to OCS metadata included:");
                var last = await dataService.GetLastValueAsync<PItoOCSType>(streamId).ConfigureAwait(false);
                Console.WriteLine(last);

                Console.WriteLine("Now let\'s use accept-verbosity as False to see the difference:");
                verbosityHeaderHandler.Verbose = false;
                last = await dataService.GetLastValueAsync<PItoOCSType>(streamId).ConfigureAwait(false);
                ...
                var windowEvents = await dataService.GetWindowValuesAsync<PItoOCSType>(streamId, startIndex, endIndex).ConfigureAwait(false);
                ...
                windowEvents = await dataService.GetWindowValuesAsync<PItoOCSType>(streamId, startIndex, endIndex).ConfigureAwait(false);
                Console.WriteLine($"Total events found: {windowEvents.Count()}");
                foreach (var value in windowEvents)
                {
                    Console.WriteLine(value.ToString());
                ...
                var rangeValues = await dataService.GetRangeValuesAsync<PItoOCSType>(streamId, startIndex, 10).ConfigureAwait(false);
                ...
                var interpolatedValues = await dataService.GetValuesAsync<PItoOCSType>(streamId, startIndex, endIndex, 10).ConfigureAwait(false);
                ...
                var filteredValues = await dataService.GetWindowFilteredValuesAsync<PItoOCSType>(streamId, startIndex, endIndex, SdsBoundaryType.Exact, $"Value lt {avgValue}").ConfigureAwait(false);
